...
class ApiClient {
  ...
  async getThreadMessages(chatId: string, threadRootId: string, page = 0): Promise<Message[]> {
  ...
  async sendReply(chatId: string, data: CreateReplyRequest): Promise<Message> {
    return this.request(`/chats/${chatId}/messages/reply`, {
      ...
      body: JSON.stringify({
          text: data.content,
          parentMessageId: data.replyMessageId
      }),
    })
  }

  async sendThreadMessage(chatId: string, data: CreateThreadRequest): Promise<Message> {
